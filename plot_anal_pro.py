...
meds_5 = []
with open('sk25_pro_FITC_4_meds_means_vars.txt') as f:
    for i in f.readlines():
        meds_5.append(float(i.split(',')[0]))
meds_6 = []
with open('sk25_pro_FITC_5_meds_means_vars.txt') as f:
    for i in f.readlines():
        meds_6.append(float(i.split(',')[0]))
meds_7 = []
with open('sk25_pro_FITC_6_meds_means_vars.txt') as f:
    for i in f.readlines():
        meds_7.append(float(i.split(',')[0]))
meds_8 = []
with open('sk25_pro_FITC_7_meds_means_vars.txt') as f:
    for i in f.readlines():
        meds_8.append(float(i.split(',')[0]))
meds_10 = []
with open('sk25_pro_FITC_8_meds_means_vars.txt') as f:
    for i in f.readlines():
...
plt.xticks([1, 2, 3, 4,5,6], [f'cntr. (n={len(data1)})', f'4.0% (n={len(data2)})', f'5.0% (n={len(data3)})', f'6.0% (n={len(data4)})', f'7.0% (n={len(data5)})',f'8.0% (n={len(data6)})'])
plt.xlabel('isopropanol conc.(%)')
plt.ylabel('meds(-)')
plt.grid(True)
fig.savefig("fig_pro_meds_240119.png",dpi = 500)
...
ax.grid(True)
ax.set_xlabel('isopropanol conc.(%)')
ax.set_ylabel('formation rate(%)')
ax.set_ylim(-5, 100)
fig.savefig('fig_pro_95_240119.png')

x_input = prob_below_lower_percentile_95_100
y_input = [0, 14.60674157, 29.41176471, 60.8974359, 100, 100]
...
fig.savefig('fig_pro_cor_240119.png')
