...
meds_c = []
with open('sk25_etapro_FITC_c_meds_means_vars.txt') as f:
...
plt.xticks([1, 2, 3, 4,5,6], [f'cntr. (n={len(data1)})', f'5.0% (n={len(data2)})', f'7.0% (n={len(data3)})', f'8.0% (n={len(data4)})', f'9.0% (n={len(data5)})',f'10.0% (n={len(data6)})'])
plt.xlabel('etanol conc.(%)')
plt.ylabel('meds(-)')
plt.grid(True)
fig.savefig("fig_eta_meds_240119.png",dpi = 500)

data1,data2,data3,data4,data5,data6= meds_c,meds_5,meds_6,meds_7,meds_8,meds_10
lower_percentile_95 = np.percentile(data1, 5)
prob_below_lower_percentile_95 = [0.05] + [np.mean(data < lower_percentile_95) for data in [data2, data3, data4, data5, data6]]
...
ax.set_ylim(-5, 100)
fig.savefig('fig_eta_95_240119.png')


x_input = prob_below_lower_percentile_95_100
y_input = [0, 14.45086705, 27.31958763, 37.59398496, 96.68874172, 100]
fig, ax =plt.subplots(figsize=(6,6))
ax.scatter(x_input[0],y_input[0],color = 'cyan',label = '0%')
ax.scatter(x_input[1],y_input[1],color = 'deepskyblue',label = '5.0%')
ax.scatter(x_input[2],y_input[2],color = 'dodgerblue',label = '7.0%')
ax.scatter(x_input[3],y_input[3],color = 'mediumslateblue',label = '8.0%')
ax.scatter(x_input[4],y_input[4],color = 'slateblue',label = '9.0%')
ax.scatter(x_input[5],y_input[5],color = 'darkviolet',label = '10.0%')  
sns.set()
ax.grid(True)
...
fig.savefig('fig_eta_cor_240119.png')
