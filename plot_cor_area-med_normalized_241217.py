...
for db_value, csv_value in zip(db_values_normalized, csv_values):
    print(f"db_value: {db_value}, csv_value: {csv_value}")

# データフレームに変換
df = pd.DataFrame({'area': db_values_normalized, 'median': csv_values})
...
# スピアマンの相関係数とp値を計算
spearman_corr, p_value = spearmanr(db_values_normalized, csv_values)
...
plt.savefig("fig_plot_cor_402_nor_area-med_241217.png")
plt.show()