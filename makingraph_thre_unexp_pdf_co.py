...
combined_min = min(data_exp.min(), data_un.min())
combined_max = max(data_exp.max(), data_un.max())
...
for r in roots:
    y = np.interp(r, bin_centers, smooth_exp)
...
plt.savefig("exp_un_pdf_common.png", dpi=300)
plt.tight_layout()
plt.show()
